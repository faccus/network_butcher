  ...
  std::map<node_id_type, node_id_type> const &link_id_nodeproto)
{
  ...
  auto const get_type = [&](std::string const &communication_node_name) {
    ...
    auto tmp_res = std::find_if(input.cbegin(),
                                input.cend(),
    ...
    if (tmp_res == original_model.graph().input().end())
      tmp_res = std::find_if(output.cbegin(),
                             output.cend(),
                             [communication_node_name](auto const &ref) {
                               return ref.name() == communication_node_name;
                             });
    ...
    if (tmp_res == original_model.graph().output().end())
      tmp_res = std::find_if(value_info.cbegin(),
                             value_info.cend(),
                             [communication_node_name](auto const &ref) {
                               return ref.name() == communication_node_name;
                             });
  ...
  auto const get_initializer = [&](std::string const &name) {
    ...
    auto const  it          = std::find_if(initializer.begin(),
                                 initializer.end(),
                                 [&name](auto const &proto_tensor) {
                                   return name == proto_tensor.name();
                                 });
    return std::pair{it == initializer.cend(), it};
  };

  auto const add_node_ios_to_graph =
    [&model_graph](onnx::GraphProto *sup_graph, onnx::NodeProto const *node) {
  ...
  auto const add_nodes = [&](std::set<node_id_type> const &nodes,
