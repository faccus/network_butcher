...
namespace network_butcher::io::IO_Manager
{
                            ...
                            const std::vector<network_butcher::types::Weighted_Real_Path> &paths)
  {
    ...
    Utilities::potentially_par_for_each(v.cbegin(), v.cend(), process_partitioning);
#else
    ...
    std::pair<bool, onnx::ModelProto> reconstruct_model_from_partition(
    ...
    void export_to_onnx(const onnx::ModelProto &model, std::string path)
    {
      network_butcher::Utilities::output_onnx_file(model, path);
    }


    std::tuple<graph_type, onnx::ModelProto, std::map<node_id_type, node_id_type>> import_from_onnx(
      std::string const &path, bool add_input_padding, bool add_output_padding, std::size_t num_devices)
    {
    ...
    network_butcher::parameters::Parameters read_parameters(const std::string &path)
    {
      GetPot file(path);
    ...
    std::unique_ptr<Weight_Importer> generate_weight_importer(graph_type & graph,
    ...
    void import_weights(graph_type & graph, const network_butcher::parameters::Parameters &params)
    {
