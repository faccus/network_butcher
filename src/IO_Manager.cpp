                                                 ...
                                                 std::size_t        num_devices)
{
  ...
  auto [value_infos, onnx_inputs_ids, onnx_outputs_ids] =
    onnx_compute_value_infos(onnx_input, onnx_output, onnx_value_info, onnx_initializer);
  ...
  for (auto const &node : onnx_nodes)
    {
      ...
      auto                                  inputs  = onnx_process_node_io(node.input(), parameters, value_infos);
      auto                                  outputs = onnx_process_node_io(node.output(), parameters, value_infos);

      auto attributes = onnx_process_node_attributes(node);
      ...
      network_butcher_types::Content<type_info_pointer> content(
        std::move(inputs), std::move(outputs), std::move(parameters), std::move(attributes), std::move(operation_type));
...
network_butcher_io::IO_Manager::onnx_process_node_attributes(const onnx::NodeProto &node)
{
...
std::tuple<network_butcher_io::IO_Manager::Map_IO, std::set<std::string>, std::set<std::string>>
network_butcher_io::IO_Manager::onnx_compute_value_infos(
...
network_butcher_io::IO_Manager::onnx_process_node_io(
