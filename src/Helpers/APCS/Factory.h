...
namespace GenericFactory
{
  /*! @brief A generic object factory.
    ...
    by the factory. It must have an ordering relation since they will be stored
    as key in a std::map
    @tparam Builder The type of the callable object that produces the the concrete
    object derived from AbstractProduct It may have arguments but its return
    type must be std::unique_ptr<AbstractProduct>.
  ...
  */
  ...
  the identifier as key.
    */
    ...
    //! The builder type.
    /*!
      The default is a function wrapper that returns a
      unique_ptr<AbstractProduct> and with no arguments.
    */
    ...
    /*!
      ...
      \param args The arguments pack to be passed to the builder. It may be
      empty \note The default builder does not take any argument. This version
      is provided in case the template is specialized with a builder that takes
    ...
    */
    Builder
    get(Identifier const &name) const;
    //
    //! Register the given rule.
    void
    add(Identifier const &, Builder_type const &);
  ...
  //! This template alias may be used to store functions: the builder is here
  //! the function type
  /*!
...
#endif // NETWORK_BUTCHER_FACTORY_H
