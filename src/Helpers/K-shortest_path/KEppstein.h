...
private:
    ...
    std::size_t                                    devices)
  {
    ...
    auto const sidetrack_distances_res =
      sidetrack_distances_linear(weights,
                                 devices,
                                 dij_res.second);                      // O(E)
    auto const shortest_path = base::shortest_path_finder(dij_res, 0); // O(N)
    ...
    H_out_map                           h_out;
    std::map<node_id_type, H_g_pointer> h_g;

    auto edges_edges = std::map<edge_type, std::set<edge_type>>();


    auto it = construct_partial_h_g_linear(
      h_g, h_out, sidetrack_distances_res, successors, 0);
    get_internal_edges(edges_edges, *it);
    ...
    for (int k = 2; k <= K && !Q.empty(); ++k)
      {
        ...
        it = construct_partial_h_g_linear(
          h_g, h_out, sidetrack_distances_res, successors, e.second);
        get_internal_edges(edges_edges, *it);
            ...
            for (std::size_t j = 0; j < devices; ++j)
              {
                ...
                if (it_dist == sidetrack_distances.cend())
                  {
                    std::cout << "Cannot find sidetrack distance for ("
                              << edge.first << ", " << edge.second << std::cout;
                    continue;
                  }
    ...
    std::map<edge_type, std::set<edge_type>> &edge_edges)
  {
...

    {
      if (node == graph.nodes.size() - 1)
        {
          auto it   = h_g.emplace(node, H_g());
          auto it_2 = construct_partial_h_out_linear(h_out,
                                                     sidetrack_distances,
                                                     successors,
                                                     node);
          it.first->second.children.insert(it_2->second);
  ...
  base_path_selector_lazy_eppstein(
    std::size_t                                     K,
    dij_res_type const                             &dij_res,
