  ...
  if (!initialized)
    {
      ...
      factory.add("relu",
                  [](Graph<graph_input_type> const &graph,
                     ...
                     bool                           forward) {
                    std::size_t res = -1;
                    auto const  it  = graph.nodes_content.find(
                      node.get_output().begin()->second);
      ...
      factory.add("batchnormalization",
                  [](Graph<graph_input_type> const &graph,
                    ...
                    auto const it2 = graph.nodes_content.find(
                      node.get_input().begin()->second);
      ...
      factory.add(
        ...
        [](Graph<graph_input_type> const &graph,
          ...
          auto const kernel_iterator =
            node.get_attributes().find("kernel_shape");
      ...
      factory.add("maxpool",
                  [](Graph<graph_input_type> const &graph,
                     ...
                     bool                           forward) {
                    std::size_t res          = 0;
                    auto const  out_iterator = graph.nodes_content.find(
                      node.get_output().begin()->second);
                    if (out_iterator == graph.nodes_content.cend())
                      return res;
                    ...
                    std::size_t const C_out =
                      out_iterator->second->get_shape()[1];

                    std::size_t const H_f_times_W_f =
                      kernel_iterator->second[0] * kernel_iterator->second[1];

                    res = forward ? H_f_times_W_f * C_out :
                                    (H_f_times_W_f + 1) * C_out;
      ...
      factory.add("add",
                  [](Graph<graph_input_type> const &graph,
                    ...
                    std::size_t const prod_out =
                      out_iterator->second->get_shape()[0] *
                      out_iterator->second->get_shape()[1] *
